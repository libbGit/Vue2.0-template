  ...
  <div class="form-wrap">
    ...
    <el-form class="form-container" size="mini" ref="form" :model="form" :rules="rules" label-width="140px">
      ...
      <el-form-item prop="name" label="广告主名称：" >
        <el-input v-model="form.name" placeholder="请输入营业执照全称" :readonly="$route.query.status!=0"></el-input>
      </el-form-item>
      ...
      <el-form-item prop="licence" label="营业执照：">
        <el-input v-model="form.licence" placeholder="请输入营业执照全称" :readonly="$route.query.status!=0"></el-input>
      </el-form-item>
...
<script>
  ...
  export default {
      ...
      onSubmit () {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            fetch.post('/advertiser/edit', this.form)
              .then( res => {
                loading.close();
                if(res.ret == 0) {
                  this.$alert('修改广告主成功', '提示', {
    ...
    created (){
      ...
      var _this = this;
      fetch.get('/advertiser/get', { params: {id: id}})
        .then( res => {
