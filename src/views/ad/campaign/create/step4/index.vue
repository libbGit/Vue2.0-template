      ...
      <div class="form-wrap" style="padding-top:0px;">
        <div class="hy-card">
          ...
          <div class="hy-card__content" style="margin-top: -15px;">
            <el-table ref="creative_table" border size="mini" :data="tableData" @selection-change="handleSelectionChange" style="width: 100%">
  ...
  export default {
    ...
    computed: {
      ...
      selected_rows () {
        return this.tableData.filter( row => {
          let currentTable = this.eachePageSection[this.currentPage];
          if(currentTable){
    ...
    methods: {
      ...
      handleSelectionChange(val){
        this.eachePageSection[0] = []; //因为是从第一页开始的，所以0的位置默认为undefined。避免出错，给初始值
        
        if(this.isHandSelected){  //初始加载完table都，都会执行此操作，会把之前选的覆盖掉，成没选择的
          console.log("this.eachePageSection===before",this.eachePageSection)
          this.eachePageSection[this.currentPage] = val;

          console.log("this.eachePageSection===after",this.eachePageSection)
        }
        ...
        fetch
          ...
          .then(res => {
            ...
            if (res.ret == 0 && Array.isArray(res.data)) {
              ...
              this.isHandSelected = false;
              console.log("fetch complete==",this.eachePageSection);

              this.$nextTick(function(){
                console.log("this.selected_rows",this.selected_rows)
                if(this.selected_rows){
                  this.$refs.creative_table.clearSelection()
                  this.selected_rows.forEach(row => {
                    this.$refs.creative_table.toggleRowSelection(row, true);
                  });
                }
                this.isHandSelected = true;  //加载完，恢复设置，否则不能手动选择
              });
      ...
      handle_save_push() {
        ...
        for(let item of this.eachePageSection){
          if(item){
