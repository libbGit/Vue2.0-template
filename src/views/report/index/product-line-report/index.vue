  ...
  <div>
    <productline-report-content 
      ...
      <data-type :slot="0" 
        ...
        title="产品线数据" 
        :headers="headerTitle" 
        :values="productlineData"
        @update="updateDateAndProductline($event)"
        @pageChange="updateProductlineData($event)">
      </data-type>
...
import productlineReportContent from "./productline-report-content";
...
export default {
  ...
  computed: {
    ...mapState({
      settings: state => state.report.settings.productline, //每次获取settings都会执行此箭头函数
  ...
  methods: {
    ...mapMutations("report", [
      ...
      "UPDATE_PRODUCTLINE_SETTING",
      "UPDATE_PRODUCTLINE_REPORT",
    ...
    fetch_get() {
      ...
      Promise.all([
        this.updateProductlineData({ currentPage: 1, limit: 5 }),
    ...
    async updateProductlineData(value) {
      this.isloading = true;
    ...
    //更新state，并加载整个界面
    updateDateAndProductline(value) {
      this.UPDATE_PRODUCTLINE_REPORT(value);
    },
    //更新数据趋势 类型
    async updateDataTrendType() {
      ...
      /*
           ...
           */
      if (this.productline_report.filterValue.length == 1) {
        this.productlineDataTrend.multiaxis = true;
        type = this.$refs.dataTrendRef.multiFilterValue;
      } else {
        this.productlineDataTrend.multiaxis = false;
      ...
      await this.fetch_advertiser_trend_data({
        ...
        if (res.data.length == 0) {
          ...
          this.productlineDataTrend.xAxisData = [];
          this.productlineDataTrend.seriesData = [];
          //this.UPDATE_PRODUCTLINE_SETTING({id:2, checked:false, disabled:true});
          return;
        }

        //this.UPDATE_PRODUCTLINE_SETTING({id:2, checked:true, disabled:false});

        let product_line = res.data.product_line;
        let xAxisData = res.data.date_arrange;
        let seriesData = [];

        for (let item in product_line) {
          this.trendTitle.push(product_line[item].name); //得到 折线图标题
          seriesData.push({
            name: product_line[item].name,
            data: product_line[item].value
          });
        }
        this.productlineDataTrend.xAxisData = xAxisData;
        this.productlineDataTrend.seriesData = seriesData;
        this.isloadingTrendData = false;
      });
    },
    //更新终端占比
    async updateDevicePercentType() {
      ...
      await this.fetch_device_percent_data({
        ...
        if (res.data.length == 0) {
          ...
          this.UPDATE_PRODUCTLINE_SETTING({
            id: 3,
            checked: false,
            disabled: true
          });
    ...
    redirectToBrandNoItem() {
      ...
      this.isloadingDevicePercent.pie = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 3,checked: false,disabled: true});

      this.isloadingDevicePercent.line = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 3,checked: false,disabled: true});

      this.isloadingAdvertiserType = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 4,checked: false,disabled: true});

      this.isloadingMediaType = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 5,checked: false,disabled: true});

      this.isloadingRegionType = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 6,checked: false,disabled: true});

      this.isloadingPos = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 7,checked: false,disabled: true});
      
      this.isloadingCrowd = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 8,checked: false,disabled: true});
      
      this.isloadingCreativeGroup = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 9,checked: false,disabled: true});

      this.isloadingCreativeData = false;
      this.UPDATE_PRODUCTLINE_SETTING({id: 10,checked: false,disabled: true});
      ...
      this.fetch_filter_data({ parentId: this.$route.query.parentId })
         ...
         */
          if (res.data.length == 0) {
            if (this.$route.path.lastIndexOf("/ad/report") != -1) {
              ...
              let interval = setInterval(() => {
                this.loadingFullScreenText = `暂无产品线记录,5秒后跳转至品牌报告...${seconds}s`;
          ...
          if (this.$route.query.parentId == undefined) {
            //
            this.UPDATE_PRODUCTLINE_REPORT({
              filterList: res.data,
              filterValue: [],
              filterKey: [],
  ...
  created() {
    ...
    this.ADD_PRODUCTLINE_SETTING(localSettings);

    this.fetchFilterData();
  },

  components: {
    productlineReportContent,
    dataType,
    dataTrend,
    devicePercent,
    advertiseType,
