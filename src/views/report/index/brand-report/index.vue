  ...
  <div>
    <brand-report-content  
      ...
      <data-type :slot="0" 
        ...
        :headers="headerTitle" 
        :values="brandData"
        @update="updateDateAndBrand($event)"
        @pageChange="updateBrandData($event)">
      </data-type>
...
import brandReportContent from "./brand-report-content";
...
export default {
  ...
  computed: {
    ...mapState({
      settings: state => state.report.settings.brand, //每次获取settings都会执行此箭头函数
  ...
  methods: {
    ...
    fetch_get() {
      ...
      Promise.all([
        this.updateBrandData({ currentPage: 1, limit: 5 }),
    ...
    //更新state, 然后重新渲染整个界面
    updateDateAndBrand(value) {
      this.UPDATE_BRAND_REPORT(value);
    },
    ...
    async updateDataTrendType() {
      ...
      /*
           ...
           */
      if (this.brand_report.filterValue.length == 1) {
        this.brandDataTrend.multiaxis = true;
        type = this.$refs.dataTrendRef.multiFilterValue;
      } else {
        this.brandDataTrend.multiaxis = false;
      ...
      await this.fetch_advertiser_trend_data({
        ...
        if (res.data.length == 0) {
          ...
          this.brandDataTrend.xAxisData = [];
          this.brandDataTrend.seriesData = [];
          //this.UPDATE_BRAND_SETTING({id:2, checked:false, disabled:true});
          return;
        }

        //this.UPDATE_BRAND_SETTING({id:2, checked:true, disabled:false});

        let brand_list = res.data.brand_list;
        let xAxisData = res.data.date_arrange;
        let seriesData = [];

        for (let item in brand_list) {
          this.trendTitle.push(brand_list[item].name); //得到 折线图标题
          seriesData.push({
            name: brand_list[item].name,
            data: brand_list[item].value
          });
        }
        this.brandDataTrend.xAxisData = xAxisData;
        this.brandDataTrend.seriesData = seriesData;
        this.isloadingTrendData = false;
      });
    },
    ...
    async updateDevicePercentType() {
      ...
      await this.fetch_device_percent_data({
        ...
        if (res.data.length == 0) {
          ...
          this.UPDATE_BRAND_SETTING({ id: 3, checked: false, disabled: true });
          return;
        }

        this.UPDATE_BRAND_SETTING({ id: 3, checked: true, disabled: false });
        let pieSeriesData = [];
        let tabletData = [];
    ...
    async updateAdvertiserType() {
      ...
      await this.fetch_advertiser_type_data({
        ...
        if (res.data.length == 0) {
          ...
          this.UPDATE_BRAND_SETTING({ id: 4, checked: false, disabled: true });
          return;
        }

        this.UPDATE_BRAND_SETTING({ id: 4, checked: true, disabled: false });
    ...
    async updateMediaType() {
      ...
      await this.fetch_media_type_data({
        ...
        if (res.data.length == 0) {
          ...
          this.UPDATE_BRAND_SETTING({ id: 5, checked: false, disabled: true });
          return;
        }

        this.UPDATE_BRAND_SETTING({ id: 5, checked: true, disabled: false });
    ...
    async updateRegionType() {
      ...
      await this.fetch_region_type_data({
        ...
        if (res.data.length == 0) {
          ...
          this.UPDATE_BRAND_SETTING({ id: 6, checked: false, disabled: true });
          return;
        }

        this.UPDATE_BRAND_SETTING({ id: 6, checked: true, disabled: false });
    ...
    async updatePointPosition() {
      ...
      await this.fetch_point_position_data({
        ...
        if (res.data.length == 0) {
          ...
          this.UPDATE_BRAND_SETTING({ id: 7, checked: false, disabled: true });
        } else {
          this.UPDATE_BRAND_SETTING({ id: 7, checked: true, disabled: false });
        }
    ...
    async updateCrowdData() {
      ...
      await this.fetch_crowd_data({
        ...
        if (res.data.tableData.length == 0) {
          this.UPDATE_BRAND_SETTING({ id: 8, checked: false, disabled: true });
        ...
        this.UPDATE_BRAND_SETTING({ id: 8, checked: true, disabled: false });
        let titles = [];
        let lineSeriesData = [];
    ...
    async updateCreativeGroup() {
      ...
      await this.fetch_creative_group_data({
        ...
        if (res.data.tableData.length == 0) {
          this.UPDATE_BRAND_SETTING({ id: 9, checked: false, disabled: true });
        ...
        this.UPDATE_BRAND_SETTING({ id: 9, checked: true, disabled: false });
    ...
    async updateCreativeData() {
      ...
      await this.fetch_creative_data({
        ...
        if (res.data.length == 0) {
          this.UPDATE_BRAND_SETTING({ id: 10, checked: false, disabled: true });
          this.creativeTableData = [];
        } else {
          this.UPDATE_BRAND_SETTING({ id: 10, checked: true, disabled: false });
        }
          ...
          if (res.data.length == 0) {
            if (this.$route.path.lastIndexOf("/ad/report") != -1) {
              ...
              let interval = setInterval(() => {
                this.loadingFullScreenText = `暂无品牌记录,5秒后跳转至广告主列表页...${seconds}s`;

                if (seconds == 0) {
                  clearInterval(interval);
                  this.$router.push({ path: "/advertiser" });
  ...
  created() {
    ...
    this.ADD_BRAND_SETTING(localSettings);
