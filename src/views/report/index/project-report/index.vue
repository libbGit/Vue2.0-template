  ...
  <div>
    <project-report-content 
      ...
      <data-type :slot="0"
        ...
        :headers="headerTitle"
        :values="projectData"
        @update="updateDateAndProject($event)"
        @pageChange="updateProjectData($event)">
      </data-type>
...
import projectReportContent from "./project-report-content";
...
export default {
  ...
  computed: {
    ...mapState({
      settings: state => state.report.settings.project, //每次获取settings都会执行此箭头函数
  ...
  methods: {
    ...
    fetch_get() {
      ...
      Promise.all([
        this.updateProjectData({ currentPage: 1, limit: 5 }),
    ...
    //更新state，并加载整个界面
    updateDateAndProject(value) {
      this.UPDATE_PROJECT_REPORT(value);
    },
    ...
    async updateDataTrendType() {
      ...
      /*
           ...
           */
      if (this.project_report.filterValue.length == 1) {
        this.projectDataTrend.multiaxis = true;
        type = this.$refs.dataTrendRef.multiFilterValue;
      } else {
        this.projectDataTrend.multiaxis = false;
      ...
      await this.fetch_advertiser_trend_data({
        ...
        if (res.data.length == 0) {
          ...
          this.projectDataTrend.xAxisData = [];
          this.projectDataTrend.seriesData = [];
          //this.UPDATE_PROJECT_SETTING({id:2, checked:false, disabled:true});
          return;
        }
        //this.UPDATE_PROJECT_SETTING({id:2, checked:true, disabled:false});

        let project_list = res.data.project_list;
        let xAxisData = res.data.date_arrange;
        let seriesData = [];

        for (let item in project_list) {
          this.trendTitle.push(project_list[item].name); //得到 折线图标题
          seriesData.push({
            name: project_list[item].name,
            data: project_list[item].value
          });
        }
        this.projectDataTrend.xAxisData = xAxisData;
        this.projectDataTrend.seriesData = seriesData;
        this.isloadingTrendData = false;
      });
    },
    ...
    async updateDevicePercentType() {
      ...
      await this.fetch_device_percent_data({
        ...
        this.UPDATE_PROJECT_SETTING({ id: 3, checked: true, disabled: false });
        let pieSeriesData = [];
        let tabletData = [];
    ...
    async updateAdvertiserType() {
      ...
      await this.fetch_advertiser_type_data({
        ...
        this.UPDATE_PROJECT_SETTING({ id: 4, checked: true, disabled: false });
    ...
    async updateMediaType() {
      ...
      await this.fetch_media_type_data({
        ...
        this.UPDATE_PROJECT_SETTING({ id: 5, checked: true, disabled: false });
    ...
    async updateRegionType() {
      ...
      await this.fetch_region_type_data({
        ...
        this.UPDATE_PROJECT_SETTING({ id: 6, checked: true, disabled: false });
    ...
    async updateCrowdData() {
      ...
      await this.fetch_crowd_data({
        ...
        this.UPDATE_PROJECT_SETTING({ id: 8, checked: true, disabled: false });
        let titles = [];
        let lineSeriesData = [];
    ...
    async updateCreativeGroup() {
      ...
      await this.fetch_creative_group_data({
        ...
        this.UPDATE_PROJECT_SETTING({ id: 9, checked: true, disabled: false });
    ...
    redirectToBrandNoItem() {
      ...
      this.isloadingDevicePercent.pie = false;
      this.UPDATE_PROJECT_SETTING({ id: 3, checked: false, disabled: true });

      this.isloadingDevicePercent.line = false;
      this.UPDATE_PROJECT_SETTING({ id: 3, checked: false, disabled: true });

      this.isloadingAdvertiserType = false;
      this.UPDATE_PROJECT_SETTING({ id: 4, checked: false, disabled: true });

      this.isloadingMediaType = false;
      this.UPDATE_PROJECT_SETTING({ id: 5, checked: false, disabled: true });

      this.isloadingRegionType = false;
      this.UPDATE_PROJECT_SETTING({ id: 6, checked: false, disabled: true });

      this.isloadingPos = false;
      this.UPDATE_PROJECT_SETTING({ id: 7, checked: false, disabled: true });

      this.isloadingCrowd = false;
      this.UPDATE_PROJECT_SETTING({ id: 8, checked: false, disabled: true });

      this.isloadingCreativeGroup = false;
      this.UPDATE_PROJECT_SETTING({ id: 9, checked: false, disabled: true });

      this.isloadingCreativeData = false;
      this.UPDATE_PROJECT_SETTING({ id: 10, checked: false, disabled: true });
    },
          ...
          if (res.data.length == 0) {
            if (this.$route.path.lastIndexOf("/ad/report") != -1) {
              ...
              let interval = setInterval(() => {
                this.loadingFullScreenText = `暂无项目记录,5秒后跳转至品牌报告...${seconds}s`;
  ...
  created() {
    ...
    this.ADD_PROJECT_SETTING(localSettings);
