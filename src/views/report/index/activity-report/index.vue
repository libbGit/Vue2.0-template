  ...
  <div>
    <activity-report-content 
      ...
      <data-type :slot="0" 
        ...
        :headers="headerTitle" 
        :values="activityData" 
        @update="updateDateAndActivity($event)"
	      @pageChange="updateActivityData($event)">
      </data-type>
      ...
      <data-trend ref="dataTrendRef" :slot="1" 
        ...
        title="活动数据趋势" 
        :headers="trendTitle" 
        :values="activityDataTrend" 
        @update="updateDataTrendType($event)">
      </data-trend>
...
import activityReportContent from "./activity-report-content";
...
export default {
  ...
  computed: {
    ...mapState({
      settings: state => state.report.settings.activity, //每次获取settings都会执行此箭头函数
  ...
  methods: {
    ...
    fetch_get() {
      ...
      Promise.all([
        this.updateActivityData({ currentPage: 1, limit: 5 }),
    ...
    async updateActivityData(value) {
      this.isloading = true;
    ...
    //更新state，并加载整个界面
    updateDateAndActivity(value) {
      this.UPDATE_ACTIVITY_REPORT(value);
    },
    ...
    async updateDataTrendType() {
      ...
      /*
           ...
           */
      if (this.activity_report.filterValue.length == 1) {
        this.activityDataTrend.multiaxis = true;
        type = this.$refs.dataTrendRef.multiFilterValue;
      } else {
        this.activityDataTrend.multiaxis = false;
      ...
      await this.fetch_advertiser_trend_data({
        ...
        if (res.data.length == 0) {
          ...
          this.activityDataTrend.xAxisData = [];
          this.activityDataTrend.seriesData = [];
          //this.UPDATE_ACTIVITY_SETTING({id:2, checked:false, disabled:true});
          return;
        }
        //this.UPDATE_ACTIVITY_SETTING({id:2, checked:true, disabled:false});

        let activity_list = res.data.activity_list;
        let xAxisData = res.data.date_arrange;
        let seriesData = [];

        for (let item in activity_list) {
          this.trendTitle.push(activity_list[item].name); //得到 折线图标题
          seriesData.push({
            name: activity_list[item].name,
            data: activity_list[item].value
          });
        }
        this.activityDataTrend.xAxisData = xAxisData;
        this.activityDataTrend.seriesData = seriesData;
        this.isloadingTrendData = false;
      });
    },
    ...
    async updateDevicePercentType() {
      ...
      await this.fetch_device_percent_data({
        ...
        this.UPDATE_ACTIVITY_SETTING({ id: 3, checked: true, disabled: false });
        let pieSeriesData = [];
        let tabletData = [];
    ...
    async updateAdvertiserType() {
      ...
      await this.fetch_advertiser_type_data({
        ...
        this.UPDATE_ACTIVITY_SETTING({ id: 4, checked: true, disabled: false });
        let pieSeriesData = [];
        let tabletData = [];
        for (let item in res.data) {
          pieSeriesData.push({
    ...
    async updateMediaType() {
      ...
      await this.fetch_media_type_data({
        ...
        this.UPDATE_ACTIVITY_SETTING({ id: 5, checked: true, disabled: false });
    ...
    async updateRegionType() {
      ...
      await this.fetch_region_type_data({
        ...
        this.UPDATE_ACTIVITY_SETTING({ id: 6, checked: true, disabled: false });
    ...
    async updateCrowdData() {
      ...
      await this.fetch_crowd_data({
        ...
        this.UPDATE_ACTIVITY_SETTING({ id: 8, checked: true, disabled: false });
        let titles = [];
        let lineSeriesData = [];
    ...
    async updateCreativeGroup() {
      ...
      await this.fetch_creative_group_data({
        ...
        this.UPDATE_ACTIVITY_SETTING({ id: 9, checked: true, disabled: false });
        let titles = [];
        let lineSeriesData = [];
        for (let item in res.data.crowd_list) {
          titles.push(res.data.crowd_list[item].name);
    ...
    redirectToBrandNoItem() {
      ...
      this.isloadingDevicePercent.pie = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 3, checked: false, disabled: true });

      this.isloadingDevicePercent.line = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 3, checked: false, disabled: true });

      this.isloadingAdvertiserType = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 4, checked: false, disabled: true });

      this.isloadingMediaType = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 5, checked: false, disabled: true });

      this.isloadingRegionType = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 6, checked: false, disabled: true });

      this.isloadingPos = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 7, checked: false, disabled: true });

      this.isloadingCrowd = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 8, checked: false, disabled: true });

      this.isloadingCreativeGroup = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 9, checked: false, disabled: true });

      this.isloadingCreativeData = false;
      this.UPDATE_ACTIVITY_SETTING({ id: 10, checked: false, disabled: true });
    },
          ...
          if (res.data.length == 0) {
            if (this.$route.path.lastIndexOf("/ad/report") != -1) {
              ...
              let interval = setInterval(() => {
                this.loadingFullScreenText = `暂无活动记录,5秒后跳转至品牌报告...${seconds}s`;
  ...
  created() {
    ...
    this.ADD_ACTIVITY_SETTING(localSettings);
